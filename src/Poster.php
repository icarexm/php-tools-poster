...
class Poster
{
    ...
    public function createPoster($background, $config = [], $fileName = '')
    {
        ...
        if (empty($backgroundPath)) {
            //背景路径错误
            throw new Exception('Background image path error');
        }
        $backgroundInfo = $this->getImgInfo($background);
        if (empty($backgroundInfo)) {
            //获取不到图片信息
            throw new Exception('Image formatting error or Imagick extension not enabled');
        }
        //资源
        ...
        if (!empty($config['text']) && is_array($config['text'])) {
            ...
            foreach ($config['text'] as $key => $val)
            {
                ...
                $fontPath = !empty($val['fontPath']) ? $val['fontPath'] : dirname(dirname(dirname(__DIR__))).'/imagefont/source/font.ttc';
                if (!is_file($fontPath)) {
                    //字体文件不存在
        ...
        //保存到本地
        imagejpeg($imageRes, $this->pathName, 90);
    ...
    public function getSrcname($rootPath = null)
    {
        ...
        //解析path
        $pathArr = explode($rootPath, $this->pathName);
    ...
    public function downloadImage($url, $fileName = '')
    {
        ...
        if (strpos($url, $this->rootPath) !== false) {
        ...
        if (empty($size[0]) || empty($size[1])) {
            ...
            //不是图片
            throw new Exception('Not a picture resource:'.$url);
        }
